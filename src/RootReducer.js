...
const initialState = { 
  ...
  { 
    link:"/",
    hash:"#Section1",
    title: "Fikirlər yalnız fikir olaraq heç nəyi dəyişməyə qadir deyil. Hacklab fikirlərinizin dünyanı dəyişə biləcəyi və gələcəyin qurulmasında rol ala biləcəyiniz texnoloji reallığa açılan qapı olacaq",
  }
  ],
  text0ru: [
  { 
    link:"/",
    hash:"#Section1",
    title: "Одни мысли ничего не могут изменить. Возможность исследовать мир и роль вашего взломанного объекта в строительстве будущего - это технологически открытая дверь.",
  }
  ],
  text01: [
  { 
    link:"/",
    hash:"#Section1",
    title: "AAmazon is rolling out a new Prime Video feature that lets users share video clips from TV shows and movies on the platform. The clips can be shared on social media.",
  }
  ],
  text02: [
  { 
    link:"/",
    hash:"#Cemiyyet",
    title: "Hacklab Cəmiyyət olaraq təkbaşınalıqdan çox, birlikdə düşünülən, analiz edilən, qərarlaşdırılan ideya və fikirləri dəstəkləyən bir cəmiyyətdir. Hacklab cəmiyyətinin maraqları “Sənaye 4.0” sahəsi ilə bağlıdır. Maraqlarımız ; Blokçeyn və Kripto (Blockchain & Crypto) , Böyük Həcmli Məlumat ( Big Data ) , Əşyaların İnterneti (IoT) ,  Laylı İstehsal (Additive Manufacturing) , Robototexnika (Robotics) sahələri üzrədir . ",
  }
  ],
  text1: [
    {
...
const RootReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    case "DELETE_CARD":
      return {
        ...state,
      search: String(action.id)
      };   
      case "AFTER_SEARCH":
      let s = state.text2.filter(
      ...
      let s0 = state.text3.filter(
        (text2) =>  text2.title.toLowerCase().includes(action.id.toLowerCase())
      );
      let s1 = state.text4.filter(
        (text2) =>  text2.title.toLowerCase().includes(action.id.toLowerCase())
      );
      let s2 = state.text5.filter(
        (text2) =>  text2.title.toLowerCase().includes(action.id.toLowerCase())
      );
      let s4 = state.text0.filter(
        (text2) =>  text2.title.toLowerCase().includes(action.id.toLowerCase())
      );
       let s4ru = state.text0ru.filter(
        (text2) =>  text2.title.toLowerCase().includes(action.id.toLowerCase())
      );
      let s5 = state.text01.filter(
        (text2) =>  text2.title.toLowerCase().includes(action.id.toLowerCase())
      );
      let s6 = state.text02.filter(
        (text2) =>  text2.title.toLowerCase().includes(action.id.toLowerCase())
      );

      let alls = s.concat(s1,s2,s0,s4,s4ru,s5,s6)
